...
async function loadLocale(lang = 'es') {
    try {
        ...
        // Verificar si la respuesta es exitosa antes de intentar leer como JSON
        if (!response.ok) {
            console.warn(`Error al cargar el archivo de idioma ${lang}.json: ${response.status}`, response);
            currentLocale = {}; // Usar valores por defecto o vacío
            return;
        }
        currentLocale = await response.json();
        applyLocalization();
    } catch (error) {
        console.error('Error al procesar el archivo de idioma:', error);
...
function displayQuestion(question) {
    // Restablecer la visibilidad de los botones al mostrar una nueva pregunta
    submitButton.style.display = ''; // Restablece a la configuración CSS por defecto
    resetButton.style.display = '';  // Restablece a la configuración CSS por defecto
    ...
    // Asignar listeners a los botones del popup DENTRO de la función que los muestra
...
function checkAnswer() {
    ...
    if (!selectedButton) {
        ...
        console.log('No se seleccionó respuesta.'); // Log
        return;
    }
    ...
    console.log(`Respuesta seleccionada: ${selectedIndex}, ¿Correcta?: ${isCorrect}`); // Log
    ...
    saveProgress();
    console.log('Progreso y respuesta guardados.'); // Log

    if (isCorrect) {
        console.log('Respuesta correcta. Procesando avance...'); // Log
        ...
        console.log('Feedback y botones actualizados. Iniciando setTimeout...'); // Log

        setTimeout(() => {
            console.log('Dentro de setTimeout. Preparando siguiente pregunta...'); // Log
            questionOverlay.style.display = 'none';
            isQuestionActive = false;
            currentQuestionIndex++;
            console.log(`currentQuestionIndex: ${currentQuestionIndex}, total questions: ${questions.length}`); // Log
            updateProgressBar(currentQuestionIndex);

            if (currentQuestionIndex < questions.length) {
                console.log('Hay más preguntas. Reproduciendo video y verificando timing.'); // Log
                player.playVideo();
                checkQuestionTiming();
            } else {
                console.log('Todas las preguntas completadas. Mostrando revisión.'); // Log
                showReviewSection();
            }
        }, 1500); // Esperar 1.5 segundos antes de continuar
    } else {
        console.log('Respuesta incorrecta.'); // Log
        feedback.textContent = getLocalizedText('incorrect_feedback');
        feedback.style.color = getComputedStyle(document.documentElement).getPropertyValue('--danger-color') || '#dc3545';
        selectedButton.classList.add('incorrect');
        
        // Mostrar una pista si la respuesta es incorrecta y hay pistas disponibles
        showHint();

        // Si es incorrecta, los botones Enviar y Reiniciar permanecen visibles.
    }
}
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    // Asignar eventos generales
    restartVideoButton.addEventListener('click', resetProgress); // Este botón sí existe al cargar

    // Los listeners de submitButton y resetButton se asignarán en displayQuestion()
});
